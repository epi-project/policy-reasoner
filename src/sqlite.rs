...
impl PolicyDataAccess for SqlitePolicyDataStore {
    ...
    fn add_version(&self, mut version: Policy, ctx: Context) -> Result<Policy, PolicyDataError> {
        use crate::schema::policies::dsl::policies;
        let mut conn = self.pool.get().unwrap();
        // get last version
    ...
    fn set_active(&self, version: i64, ctx: Context) -> Result<Policy, PolicyDataError> {
        use crate::schema::active_version::dsl::active_version;
        let mut conn = self.pool.get().unwrap();
